...
import { Plus, Search, Edit, Trash2, Calendar, MapPin, Users, XCircle, CalendarPlus, Type, Mic2, Image as ImageIcon, UploadCloud, Eye, EyeOff, Loader2, Check } from "lucide-react";
...
export default function HREventsManagement() {
  ...
  const openAttendees = async e => {
    setAttendeesEvent(e);
    setAttendeesLoading(true);
    try {
      const res = await fetch(`/api/events/${e.id}/rsvp?all=true`);
  ...
  const handleDelete = async id => {
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}));
      alert(body.error || "Could not delete this event.");
      return;
    }
    await loadEvents();
  };
  ...
  return <>
      <PageHeader title="Events Management" description="Create, manage, and publish alumni events." breadcrumbs={[{
      label: "HR Portal"
    }, {
      ...
      <div className="space-y-4">
        ...
        {!loading && filtered.length === 0 && <p className="text-sm text-muted-foreground">No events found.</p>}
        {filtered.map(e => <Card key={e.id} className="border-border hover:shadow-sm transition-all">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground flex-wrap">
                        ...
                        year: "numeric"
                          ...
                          {e.event_time ? ` at ${formatEventTimeDisplay(e.event_time)}` : ""}
                        </span>
                        <span className="flex items-center gap-1"><MapPin className="h-3 w-3" />{e.location}</span>
                        <button className="flex items-center gap-1 hover:text-primary hover:underline" onClick={() => openAttendees(e)}>
                          <Users className="h-3 w-3" />{e.attendees} RSVP
                        </button>
                      </div>
                    </div>
      ...
      <Dialog open={!!attendeesEvent} onOpenChange={o => !o && setAttendeesEvent(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Attendees</DialogTitle>
            <p className="text-sm text-muted-foreground">{attendeesEvent?.title}</p>
          </DialogHeader>
          <div className="max-h-80 overflow-y-auto space-y-2">
            {attendeesLoading && <p className="text-sm text-muted-foreground text-center py-4">Loading...</p>}
            {!attendeesLoading && attendees.length === 0 && <p className="text-sm text-muted-foreground text-center py-4">No RSVPs yet.</p>}
            {!attendeesLoading && attendees.map(a => <div key={a.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-secondary/40">
