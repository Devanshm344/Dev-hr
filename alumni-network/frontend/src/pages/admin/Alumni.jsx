...
import { Search, Eye, Edit, Trash2, Download, UserX, UserCheck } from "lucide-react";
...
export default function AdminAlumni() {
  ...
  return <>
      <PageHeader title="Alumni Management" description="System-level management of all alumni records." breadcrumbs={[{
      label: "Admin Portal"
    }, {
      label: "Alumni Management"
    }]}>
        <Button variant="outline" size="sm" className="h-9 gap-1.5" onClick={handleExport}><Download className="h-4 w-4" />Export All</Button>
      </PageHeader>
      ...
      <Card className="border-border">
        <CardHeader className="pb-3">
          <CardTitle className="text-base">{total} Alumni Records</CardTitle>
        </CardHeader>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-secondary/30">
                  {["Alumni", "Email", "LinkedIn", "Exited Year", "Industry", "Status", "Actions"].map(h => <th key={h} className="text-left text-xs font-medium text-muted-foreground uppercase tracking-wide px-6 py-3">{h}</th>)}
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {loading && <tr><td colSpan={7} className="px-6 py-8 text-center text-sm text-muted-foreground">Loading...</td></tr>}
                {!loading && alumni.length === 0 && <tr><td colSpan={7} className="px-6 py-8 text-center text-sm text-muted-foreground">No alumni found.</td></tr>}
                {alumni.map(a => <tr key={a.id} className="even:bg-muted/30 hover:bg-primary/5 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <span className="block text-sm font-medium text-foreground">{a.full_name}</span>
                          <span className="block text-xs text-muted-foreground">{a.designation || "—"}{a.employer ? ` at ${a.employer}` : ""}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-xs text-muted-foreground">{a.email}</td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex gap-1">
                        ...
                        {a.status === "active" ? <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-orange-600" onClick={() => handleToggleActive(a)}><UserX className="h-3.5 w-3.5" /></Button> : <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-emerald-600" onClick={() => handleToggleActive(a)}><UserCheck className="h-3.5 w-3.5" /></Button>}
