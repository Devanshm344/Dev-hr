...
import { Bell, BellOff, CheckCheck, UserCheck, Quote, ClipboardList, Calendar, UserPlus, MessageSquare, MessageCircle } from "lucide-react";
...
const typeColor = {
  account: "text-blue-500 bg-blue-100 dark:bg-blue-900/30",
  story: "text-amber-500 bg-amber-100 dark:bg-amber-900/30",
  request: "text-purple-500 bg-purple-100 dark:bg-purple-900/30",
  event: "text-emerald-500 bg-emerald-100 dark:bg-emerald-900/30",
  connection: "text-sky-500 bg-sky-100 dark:bg-sky-900/30",
  message: "text-pink-500 bg-pink-100 dark:bg-pink-900/30",
  forum: "text-indigo-500 bg-indigo-100 dark:bg-indigo-900/30"
};
export default function NotificationsPage() {
  ...
  const load = () => {
    setLoading(true);
    fetch("/api/notifications").then(res => res.json()).then(data => setNotifs(data.notifications ?? [])).catch(() => {}).finally(() => setLoading(false));
  };
  useEffect(() => {
    load();
  }, []);
  ...
  return <>
      <PageHeader title="Notifications" description="Stay up to date with your alumni portal activity." breadcrumbs={[{
      label: "Portal"
    }, {
      label: "Notifications"
    }]}>
      ...
      <div className="space-y-2 max-w-2xl">
        ...
        const iconClass = typeColor[n.type] ?? typeColor.account;
